 ...
 * This unit contains the main function of the Interface Monitor portion of the
 * assignment.
 */
...
int main(int argc, char *argv[]) {
  ...
  if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
    std::cout << "Client " << getpid() << ", Error: " << strerror(errno)
              << std::endl;
    exit(-1);
  }
  ...
  if (ret < 0) {
    std::cout << "Client " << getpid() << ": error reading socket."
              << std::endl;
  ...
  while (is_running) {
    if (strcmp(buffer, "Quit") == 0) { // Server requests the client to
                                       // terminate
      std::cout << "client(" << getpid() << "): received request to quit"
                << std::endl;
      is_running = false;
    }
