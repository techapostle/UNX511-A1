 ...
 * This unit contains the utilities shared across the Network Interface Monitor units.
*/
...
int set_if_flags(char *ifname, short flags) {
  ...
  if (res < 0) {
    std::cout << "Interface " << ifname << " Error: SIOCSIFFLAGS failed: "
      << strerror(errno);
  } else {
...
void getInterfaceStat(char *interface, std::string &data) {
  ...
  data = "Interface: " + tmp_interface + " state: " + tmp_operstate + " up_count: " + std::to_string(carrier_up_count) + "\n" +
	   " rx_bytes: " + std::to_string(rx_bytes) + " rx_dropped: " + std::to_string(rx_dropped) + " rx_errors: " + std::to_string(rx_errors) +
	   " rx_packets: " + std::to_string(rx_packets) + " \n tx_bytes: " + std::to_string(tx_bytes) +
	   " tx_dropped: " + std::to_string(tx_dropped) + " tx_errors: " + std::to_string(tx_errors) + " \n\n";
}
}
